...
namespace AsiaticIndustries.Core.Infrastructure.DataProvider
{
    public class BaseDataProvider
    {
       // private readonly Database _db;
        ...
        {
            _db = new Database("AsiaticIndustries");
        }

       public ServiceResponse SaveObject<T>(T obj, long loggedInUserId, string successMessage = Constants.RecordSavedSuccessfully, bool checkForUniqueValues = true) where T : class, new()
       {
            ...
            //    {
            //        response.Message = string.Format(Constants.RecordAlreadyExists, uniqueValue);
        ...
        public ServiceResponse SaveUniqueAndNotIsDelete<T>(T obj,string successMessage = Constants.RecordSavedSuccessfully, bool checkForUniqueValues = true) where T : class, new()
        {
            ServiceResponse response = new ServiceResponse();
           
            if (checkForUniqueValues)
            {
                string uniqueValue = CheckForUniqueAndNotIsDeletedValues(obj);
                if (!string.IsNullOrEmpty(uniqueValue))
                {
                    response.Message = string.Format(Constants.RecordAlreadyExists, uniqueValue);
                    if (uniqueValue.Contains(","))
                ...
                if (null != prop && prop.CanWrite)
                {
                    foreach (SetUserNameAttribute attr in Attribute.GetCustomAttributes(prop).OfType<SetUserNameAttribute>())
                    {
                        ...
                        string employeeName = userIDWithName.ContainsKey(userID)
                                                  ...
                                                  : GetEntity<User>(userID).FirstName;
        ...
        public string CheckForUniqueAndNotIsDeletedValues<T>(T item) where T : class, new()
        {
            List<SearchValueData> searchParam = new List<SearchValueData>();
                        ...
                        foreach (SetValueOnAddAttribute attr in Attribute.GetCustomAttributes(prop).OfType<SetValueOnAddAttribute>())
                        {
                            switch (attr.SetValue)
                            {
                                case (int)Common.SetValue.CurrentTime:
                                    prop.SetValue(item, DateTime.Now, null);
                                    break;
        ...
        public ServiceResponse GetPageRecords<T>(int pageSize, int pageIndex, string sortIndex, string sortDirection, List<SearchModel> searchModelList, string customFilter = null) where T : class, new()
        {
            ServiceResponse response = new ServiceResponse();
            //if (searchModel != null && !string.IsNullOrEmpty(searchModel.Value))
            //    searchModel.Value = searchModel.Value.Replace("'", "''");
        ...
        public T GetMultipleEntity<T>(string query, params object[] args) where T : new()
        {
            ...
            {
                if (!Attribute.IsDefined(info, typeof(IgnoreAttribute)))
                {
